      SUBROUTINE WLC_SIMULATION_POSZ(X,Y,Z,PL,BL,NM)
      REAL(8) X, Y, Z, PL, BL
      INTEGER(4)NM
      INTENT(IN) PL, BL, NM
      INTENT(OUT) X, Y, Z
...
C       NM  - Number of monomers in the polymer
C 
C     Outputs:
C       X, Y, Z - Coordinates of the polymer chain end (Z>0)
C 
C     WLC theory:
C 
...
C     ------------------------------------------------------------------
      REAL(8) TWOPI, CORR_FAC, U, EXPU, DIFFEU, INV_LANGEVIN
